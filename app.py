...
if mode == "单一产品详情":
                ...
                for step in steps:
                    for param in step.get("关键参数", []):
                        all_params.append({
                            "参数名称": param,
                            "所属步骤": step["name"],
                            "参数类型": self._classify_parameter(param),
                            "重要程度": self._assess_importance(param)
                        })
                ...
                for step in steps:
                    for equip in step.get("设备", []):
                        equipment_data.append({
                            "设备名称": equip,
                            "使用步骤": step["name"],
                            "设备类型": self._classify_equipment(equip),
                            "使用频率": 1
                        })
    ...
    # 辅助方法
    @staticmethod
    def _classify_parameter(param_name):
        """参数分类"""
    ...
    @staticmethod
    def _assess_importance(param_name):
        """评估参数重要程度"""
        param_lower = param_name.lower()
    ...
    @staticmethod
    def _classify_equipment(equip_name):
        """设备分类"""
